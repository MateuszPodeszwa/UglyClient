...
//TODO UNUSED DIRECTIVES

namespace BeautifulClient;

class OldProgram //TODO EXPLICIT MODIFIERS, FUNCTIONS WRITTEN WITH NO UNIT TESTING IN MIND
{
    //TODO WHY SO MANY TASKS
    static async Task OldMain(string[] args)
    {
        ...
        const string apiKey = "u007-key"; //TODO EXPOSED API KEY

        client.DefaultRequestHeaders.Add("X-Api-Key", apiKey);

        while (true) //TODO BAD DESIGN/TOO LONG/MENU SUBJECT TO CHANGE?
        {
            //TODO MANUAL NUMBERING
            // TODO IS IT CONCREATE IO CHANNEL WHAT ABOUT THE POTENTIAL IMPLEMENTATION
            ...
            var input = Console.ReadLine(); //TODO WHAT IF NULL
            
            //TODO HARD TO READ CODE/LAMDA SWITCH EXPRESSIONS?
            switch (input)
            {
                //TODO BADLY NESTED CODE
                case "1":
                
                    Console.Write("Enter Fan Number: ");
                    if (int.TryParse(Console.ReadLine(), out int fanId)) //TODO DRY
                    {
                        Console.Write("Turn Fan On or Off? (on/off): ");
                        var stateInput = Console.ReadLine();
                        bool isOn = stateInput?.ToLower() == "on";

                        //TODO IF LAMBDA SWITCH EMPLOYED, CREATE EXTERNAL FUNCTION TO CATCH ERRORS
                        //TODO DRY
                        try
                        {
                ...
                case "2":
                    ...
                    if (int.TryParse(Console.ReadLine(), out int heaterId))
                    {
                        ...
                        //TODO USE PATTERNS FOR BETTER REDABILITY
                        if (int.TryParse(Console.ReadLine(), out int level) && level >= 0 && level <= 5)
                        {
                            try //TODO INSTEAD OF WRITING MANY TRY CATCHES ENCAPSULATE WHOLE FUNCTION WITH IT, ARE THESE EVEN NECESSARY?
                            {
                ...
                case "4":
                    ...
                    try
                    {
                        ...
                        for (int i = 1; i <= 3; i++) //TODO Assuming there are 3 heaters for this example
                        {
                            var heaterResponse = await client.GetAsync($"api/heat/{i}/level"); //TODO WHY IS IT HERE
                            if (heaterResponse.IsSuccessStatusCode)
                            {
                                ...
                                {
                                    Console.WriteLine($"  Heater {i}: Failed to parse level."); //TODO CONSIDER CUSTOM EXCEPTIONS?
                                }
                            }
                            else
                            {
                ...
                case "5":
                    ...
                    //TODO WHERES THE READLINE??
                    double currentTemperature = await GetAverageTemperature(client); // TODO STRICTLY TYPEDE IS FINE BUT YOU USE MIX OF VAR, DECIDE FOR ONE
                    while (true) //TODO HOW DO YOU ESCAPE IT??
                    {
                        //TODO WHAT IS EVEN THE PURPOSE OF THIS IT DOESNT MAKE ANY SENSE
                        
                        // Phase 1: Gradually increase to 20°C over 30 seconds
                        
                        //TODO ADJUST AND HOLD COULD POSSIBLY BE ONE OVERLOADED FUNCTION
                ...
                //TODO YOU CANT TRUST THE API CALL? MAKE IT IN ONE PLACE NOT SCATTERED ALL OVER THE PLACE
                case "6":
                    // await Reset(client);
                    Console.WriteLine("Resetting client state...");
                    //TODO UI IS KINDA UXN'T
                    try
                    {
            ...
            //TODO INSTEAD CLEAN CONSOLE, IF LOG NECESSARY CREATE FILE WITH HISTORY
        }
    }
    ...
    static async Task<double> GetAverageTemperature(HttpClient client)
    {
        ...
        //TODO WHY IS ONE DOIBLE, INT ETC..
    ...
    {
        for (int i = 1; i <= 3; i++) //TODO Assuming 3 fans
        {
    ...
    //TODO WHY 3 SENSORS MAKE IT ONE FUCNTION WHY IF YOU GET MORE SENSORS IN THE FUTURE
    static async Task<string> GetSensor1Temperature(HttpClient client)
    {
        ...
        //TODO SEPARATE IT INTO FILE TO REUSE
    ...
    static async Task<decimal> GetSensor3Temperature(HttpClient client)
    {
        ...
        //TODO INVERT IF FOR BETTER READABILITY AND NESTING
        if (response.IsSuccessStatusCode)
        {
            var content = await response.Content.ReadAsStringAsync();
...
//TODO WHAT IS THIS EVEN DOING, WHY NO XML OR ANY COMMENTS HERE?
    public class FanDTO
    {
